...
class DevToolsManager:
    ...
    def _verify_app_installed(self, app_record: dict) -> bool:
        """Verify that an app is genuinely installed on the local system."""
        # 1. Check known system tool definitions
        app_id = (app_record.get("app_id") or "").lower()
        name = (app_record.get("name") or "").lower()

        # Check binaries in definition
        for t in _KNOWN_SYSTEM_TOOLS:
            if t["app_id"] == app_id or t["name"].lower() == name:
                for b in t.get("binaries", []):
                    if shutil.which(b) or shutil.which(f"{b}.exe"):
                        return True
                return False

        # 2. General binary probe based on app_id or name
        probe_names = [app_id, name.replace(" ", "-"), name.replace(" ", "")]
        for p in probe_names:
            if shutil.which(p) or shutil.which(f"{p}.exe"):
                return True

        # If user explicitly marked it as managed and provided an install/update cmd, probe command binary
        up_cmd = app_record.get("update_command") or ""
        first_token = up_cmd.split()[0] if up_cmd else ""
        if first_token and (shutil.which(first_token) or shutil.which(f"{first_token}.exe")):
            return True
    ...
    def list_managed_apps(self) -> list:
        """
        Return all tracked managed apps verified to be genuinely installed on this machine.
        """
        combined = {}
        ...
        # 2. Layer user recorded apps, but ONLY if they are genuinely present on the system
        for app_id, app in self._apps.items():
            if self._verify_app_installed(app):
                combined[app_id] = app
