...
def diff_snapshots(snap_id_1: str, snap_id_2: str) -> Dict[str, Any]:
    """Compare two snapshots and return package/environment differences."""
    manifest = _load_manifest()
    s1 = next((s for s in manifest if s["id"] == snap_id_1), None)
    s2 = next((s for s in manifest if s["id"] == snap_id_2), None)
    if not s1 or not s2:
        return {"ok": False, "error": "One or both snapshots not found"}

    p1 = Path(s1.get("path", "")) / "snapshot.json"
    p2 = Path(s2.get("path", "")) / "snapshot.json"
    d1 = json.loads(p1.read_text(encoding="utf-8")) if p1.exists() else s1
    d2 = json.loads(p2.read_text(encoding="utf-8")) if p2.exists() else s2

    pkgs1 = d1.get("installed_packages", {})
    pkgs2 = d2.get("installed_packages", {})

    all_pms = set(pkgs1.keys()) | set(pkgs2.keys())
    package_diff: Dict[str, Any] = {}

    for pm in all_pms:
        set1 = set(pkgs1.get(pm, []))
        set2 = set(pkgs2.get(pm, []))
        package_diff[pm] = {
            "added": list(set2 - set1)[:50],
            "removed": list(set1 - set2)[:50],
            "unchanged_count": len(set1 & set2),
        }

    env1 = d1.get("env_vars", {})
    env2 = d2.get("env_vars", {})
    env_diff = {
        "modified_keys": [k for k in env1 if k in env2 and env1[k] != env2[k]],
        "added_keys": list(set(env2.keys()) - set(env1.keys())),
        "removed_keys": list(set(env1.keys()) - set(env2.keys())),
    }

    return {
        "ok": True,
        "snap_1": {"id": s1["id"], "label": s1.get("label", ""), "created_at": s1.get("created_at")},
        "snap_2": {"id": s2["id"], "label": s2.get("label", ""), "created_at": s2.get("created_at")},
