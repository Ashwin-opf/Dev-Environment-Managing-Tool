"""
macos_machine_state.py — macOS Host Machine State Assessment.

Queries macOS host subsystem for machine-state signals:
- Pending reboot detection.
- Homebrew and resource concurrency locks.
...
logger = logging.getLogger("pc_doctor.platform.macos.machine_state")
...
class MacOSMachineStateProvider(MachineStateProvider):
    """macOS-specific machine state provider querying system flags and host telemetry."""

    def check_pending_reboot(self) -> Optional[bool]:
        """Checks if a macOS restart is pending."""
        if platform.system() != "Darwin":
            return None
        return False

    def check_resource_lock(self, resource_name: Optional[str] = None) -> Optional[bool]:
        """Checks if Homebrew or custom resource lock is active."""
        try:
    ...
    def check_package_manager_available(self, package_manager: str) -> Optional[bool]:
        """Checks if package manager (brew) exists on PATH."""
    ...
    def collect_machine_signals(self, target_identity: Optional[str] = None) -> Dict[str, Any]:
        """Collects complete macOS host telemetry into a normalized dictionary."""
        ...
        try:
            ...
            logger.warning("Failed to query macOS disk usage: %s", exc)
        ...
        return {
            ...
            "package_manager_available": self.check_package_manager_available("brew"),
