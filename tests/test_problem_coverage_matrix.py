...
PROBLEM_COVERAGE_MATRIX: Dict[int, Dict[str, Any]] = {
    ...
    35: {
        ...
        "primary_mechanism": "Managed Footprint & Residual Scanner + Execution Engine",
        "detection": "ManagedFootprintRegistry + ResidualScanner probes files, configs, cache, PATH, services",
        "diagnosis": "Package manager uninstalls binary but retains configuration, environment, or residual files",
        "recipe_mechanism": "ResidualCleanupCoordinator compiles CLEANUP ExecutionPlan for Centralized Execution Engine",
        "safety_mechanism": "ResidualSafetyPolicy protects user data and mixed directories; Tier 2/3 approval required",
        "execution_mechanism": "Centralized Execution Engine (RecipeOperation.CLEANUP)",
        "verification_mechanism": "Post-cleanup verification probe (fs.exists, PATH check, service query, rescan)",
        "recovery_mechanism": "Pre-cleanup preview with SAFE_CLEANUP vs REVIEW_REQUIRED and honest rollback reporting",
        "status": "IMPLEMENTED"
    },
    36: {
    ...
    71: {
        ...
        "category": "DOCUMENTATION / KNOWLEDGE",
        "primary_mechanism": "A. Canonical Identity & OfficialUrlValidator (Trusted Source Intelligence)",
        "detection": "OfficialUrlValidator probes URL and redirect chain (max 5 hops) against canonical trusted domains, detects loops and SSRF",
        "diagnosis": "Publisher website migrated, redirected, or attempts invalid downgrade/SSRF",
        "recipe_mechanism": "Resolves verified destination URL or falls back to trusted canonical identity official_url",
        "safety_mechanism": "Rejects SSRF destinations (loopback, private, link-local) and HTTPS->HTTP downgrades",
        "execution_mechanism": "Execution Engine",
        "verification_mechanism": "OfficialUrlValidator probe",
